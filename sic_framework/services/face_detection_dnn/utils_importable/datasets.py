...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        try:
            ...
            self.img_files = [
                x.replace("/", os.sep).split(" ")[0]
                for x in f
                if x.split(" ")[0].split(".")[-1].lower() in img_formats
            ]
            sorted_index = [
                i[0] for i in sorted(enumerate(self.img_files), key=lambda x: x[1])
            ]
            self.img_files = [self.img_files[index] for index in sorted_index]
            if self.tidl_load:
                self.img_sizes = [
                    x.replace("/", os.sep).split(" ")[2].split(",")
                    for x in f
                    if x.split(" ")[0].split(".")[-1].lower() in img_formats
                ]
                self.img_sizes = [self.img_sizes[index] for index in sorted_index]
                self.img_sizes = [
                    [int(dim_size) for dim_size in img_size]
        ...
        if cache_path.is_file():
            ...
            if (
                ...
                cache, exists = (
                    self.cache_labels(cache_path, prefix, self.kpt_label),
                    False,
                )  # re-cache
        else:
            cache, exists = (
        ...
        if self.rect:
            ...
            if not tidl_load:
                ...
                self.batch_shapes = (np.array(shapes) * img_size / stride + pad).astype(
                    np.int
                ) * stride
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            ...
                            ), "labels require {} columns each".format(
                                kpt_label * 3 + 5
                            )
                            assert (
                                l[:, 5::3] <= 1
                            ...
                            for i in range(len(l)):
                                kpt = np.delete(
                                    l[i, 5:], np.arange(2, l.shape[1] - 5, 3)
                                )  # remove the occlusion paramater from the GT
                                kpts[i] = np.hstack((l[i, :5], kpt))
                            l = kpts
                            assert (
                                l.shape[1] == kpt_label * 2 + 5
                            ), "labels require {} columns each after removing occlusion paramater".format(
                                kpt_label * 2 + 5
                            )
                        else:
                        ...
                        ne += 1  # label empty
                        l = (
                            np.zeros((0, kpt_label * 2 + 5), dtype=np.float32)
                            if kpt_label
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if self.tidl_load:
                h0, w0 = self.img_sizes[index][
                    :-1
                ]  # modify the oroginal size for tidll loaded images
            # Letterbox
            shape = (
        ...
        labels_out = (
            torch.zeros((nL, 6 + 2 * self.kpt_label))
            if self.kpt_label
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            labels[:, 1:] = xywhn2xyxy(
                labels[:, 1:], w, h, padw, padh, kpt_label=self.kpt_label
