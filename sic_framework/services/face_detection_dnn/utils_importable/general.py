...
def scale_coords(
    img1_shape, coords, img0_shape, ratio_pad=None, kpt_label=False, step=2
):
...
def non_max_suppression(
    ...
    if nc is None:
        nc = (
            prediction.shape[2] - 5
            if not kpt_label
            else prediction.shape[2] - 5 - kpt_label * 3
        )  # number of classes
...
def non_max_suppression_export(
    ...
    xc = prediction[..., 4] > conf_thres  # candidates
    output = [
        torch.zeros((0, kpt_label * 3 + 6), device=prediction.device)
    ] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        x = x[xc[xi]]  # confidence
        # Compute conf
